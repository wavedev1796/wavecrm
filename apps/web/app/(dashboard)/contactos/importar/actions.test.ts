...
test("reenvía solo el archivo y el mapeo y anuncia cuántos contactos entraron", async () => {
  ...
  expect((await importContacts(null, form(csv)))?.message).toBe(
...
test("un rechazo o un fallo de red se explican; una sesión vencida sigue al login", async () => {
  api.mockResolvedValueOnce(
    Response.json(
      { error: { message: "Asigna la columna del apellido." } },
      { status: 400 },
    ),
  );
  expect(await importContacts(null, form(csv))).toEqual({
    tone: "error",
    message: "Asigna la columna del apellido.",
    errors: [],
  });
  ...
  await expect(importContacts(null, form(csv))).rejects.toThrow(
    "NEXT_REDIRECT /login?sesion=expirada",
  );
});

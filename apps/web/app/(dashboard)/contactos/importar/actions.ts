...
export type ImportState = { tone: 'success' | 'error'; message: string; errors: RowError[] } | null;

const failure = (message: string, errors: RowError[] = []): ImportState => ({ tone: 'error', message, errors });

export async function importContacts(_state: ImportState, formData: FormData): Promise<ImportState> {
  const file = formData.get('file');
  // Sin archivo elegido, el navegador envía un File vacío.
  if (!(file instanceof File) || !file.size) return failure('Adjunta un archivo CSV.');
  ...
  try {
    const response = await authenticatedApi('/contacts/import', { method: 'POST', body });
    if (response.status === 422) {
      const { error } = (await response.json()) as { error: { message: string; errors: RowError[] } };
    ...
    const message = imported === 1 ? 'Se importó 1 contacto.' : `Se importaron ${imported} contactos.`;

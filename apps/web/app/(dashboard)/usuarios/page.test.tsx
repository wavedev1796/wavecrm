...
const people = [
  ...
  },
  {
    ...
    active: true,
    status: "active",
    invitationSentAt: "2026-09-10T12:00:00.000Z",
    invitationExpiresAt: null,
  },
  {
    id: "u3",
    name: "Luis Pérez",
    email: "luis@empresa.ec",
    role: "VENDEDOR",
    active: false,
    status: "pending",
    invitationSentAt: "2026-09-18T12:00:00.000Z",
    invitationExpiresAt: "2026-09-20T12:00:00.000Z",
  },
  {
    id: "u4",
    name: "Eva Ruiz",
    email: "eva@empresa.ec",
    role: "VENDEDOR",
    active: false,
    status: "inactive",
    invitationSentAt: null,
  ...
  api.mockImplementation(async (path: string) =>
    path === "/auth/me"
      ...
      : (list ??
        Response.json({
          data: people,
          meta: { page: 1, limit: 10, total: people.length, totalPages: 1 },
        })),
  );
}
...
const row = (email: string) =>
  within(screen.getByRole("row", { name: new RegExp(email) }));
...
test("lista usuarios con su estado y solo las acciones que corresponden", async () => {
  ...
  await open();
  expect(
    row("admin@wave.ec").queryByRole("button", { name: "Desactivar" }),
  ).toBeNull();
  expect(
    row("admin@wave.ec").queryByRole("button", { name: "Eliminar" }),
  ).toBeNull();
  expect(
    row("ana@empresa.ec").getByRole("button", { name: "Desactivar" }),
  ).toBeInTheDocument();
  expect(
    row("luis@empresa.ec").getByText("Invitación pendiente"),
  ).toBeInTheDocument();
  expect(
    row("luis@empresa.ec").getByRole("button", { name: "Reenviar invitación" }),
  ).toBeInTheDocument();
  expect(
    row("eva@empresa.ec").getByRole("button", { name: "Reactivar" }),
  ).toBeInTheDocument();
...
test("pasa al API solo filtros válidos y limita la búsqueda a 100 caracteres", async () => {
  ...
  expect(api).toHaveBeenCalledWith("/users?page=1&limit=10&search=ana");
  expect(
    screen.getByRole("textbox", { name: "Buscar usuario" }),
  ).toHaveAttribute("maxlength", "100");
});

test("pagina resultados y conserva la búsqueda y el estado en los enlaces", async () => {
  respond({
    list: Response.json({
  ...
  await open({ search: "ana", status: "active", page: "2" });

  expect(api).toHaveBeenCalledWith(
    "/users?page=2&limit=10&search=ana&status=active",
  );
  expect(screen.getByText(/Mostrando 11–12 de 25/)).toBeInTheDocument();
  expect(screen.getByRole("link", { name: "Anterior" })).toHaveAttribute(
    "href",
    "/usuarios?search=ana&status=active&page=1",
  );
...
test('si la lista no carga lo dice, en vez de mostrar "no hay usuarios"', async () => {
  respond({
    list: Response.json({ error: { message: "x" } }, { status: 500 }),
  });
  await open();
  ...
  expect(
    screen.queryByText("No hay usuarios que coincidan con el filtro."),
  ).toBeNull();
});
